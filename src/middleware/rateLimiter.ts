...
export class RateLimiter {
  ...
  middleware = async (
    ...
    try {
      ...
      console.log(
        `🔍 Redis sliding window rate limiting check for vehicle ${vehicleVin} on ${endpoint}`,
      );
      ...
      res.set({
        ...
        "X-RateLimit-Window-Start": Math.ceil(
          result.windowStart / 1000,
        ).toString(),
        "X-RateLimit-Requests-In-Window": result.count.toString(),
        ...(result.averageInterval && {
          "X-RateLimit-Avg-Interval": Math.round(
            result.averageInterval,
          ).toString(),
        }),
      });
      ...
      if (!result.allowed) {
        const nextAllowedTime =
          Math.min(...result.requestTimes) + this.config.windowMs;
        ...
        console.log(
          `🚫 Sliding window rate limit exceeded for vehicle ${vehicleVin}: ${result.count}/${this.config.maxRequests}`,
        );
        console.log(
          `📊 Request pattern: ${result.requestTimes.length} requests in last ${this.config.windowMs / 1000}s`,
        );
        if (result.averageInterval) {
          console.log(
            `⏱️ Average interval between requests: ${Math.round(result.averageInterval)}ms`,
          );
        }
          ...
          slidingWindow: {
            ...
            requestPattern: {
              ...
              averageInterval: result.averageInterval
                ? Math.round(result.averageInterval)
                : null,
              requestTimes: result.requestTimes
                .slice(-5)
                .map((t) => new Date(t)),
            },
          },
      ...
      console.log(
        `✅ Request allowed for vehicle ${vehicleVin} (${result.count}/${this.config.maxRequests})`,
      );
      console.log(
        `📈 Sliding window: ${result.requestTimes.length} requests, ${result.remaining} remaining`,
      );
      next();
    } catch (error) {
      console.error(
        "Redis sliding window rate limiting error, falling back to allow:",
        error,
      );
      next();
    }
  ...
  static createTelemetryLimiter(): RateLimiter {
    return new RateLimiter({
      ...
      maxRequests: 5,
      message:
        "Too many telemetry requests. Maximum 5 requests per 10 seconds allowed.",
    });
  }
  ...
  static createBatchLimiter(): RateLimiter {
    return new RateLimiter({
      ...
      maxRequests: 2,
      message:
        "Too many batch requests. Maximum 2 batch operations per minute allowed.",
    });
  }
}
...
export class RateLimitMonitor {
  ...
  static async getSlidingWindowStats(
    vehicleVin: number,
    endpoint?: string,
  ): Promise<any> {
    ...
    if (!endpoint) {
      ...
      return {
        ...
        slidingWindow: {
          ...status,
          requestPattern:
            status.requestTimes.length > 1
              ? {
                  intervals: status.requestTimes
                    .slice(1)
                    .map((time, i) => time - status.requestTimes[i]),
                  averageInterval:
                    status.requestTimes.length > 1
                      ? (status.requestTimes[status.requestTimes.length - 1] -
                          status.requestTimes[0]) /
                        (status.requestTimes.length - 1)
                      : null,
                }
              : null,
    ...
    try {
      ...
      const summary = {
        ...
        averageRequestsPerWindow:
          validStats.length > 0
            ? validStats.reduce((sum, s) => sum + s.count, 0) /
              validStats.length
            : 0,
        mostActiveVehicle:
          validStats.length > 0

...
router.post("/vehicle/provision", async function (req: any, res: any) {
  try {
    ...
    if (!apiKey) {
      return res.status(400).json({
        error:
          "Failed to provision vehicle. Vehicle may not exist or already be provisioned.",
      });
    }
    ...
    res.json({
      ...
      apiKey,
      instructions:
        "Store this API key securely in the vehicle's onboard computer. Include it as 'X-Vehicle-API-Key' header in all telemetry requests.",
    });
  } catch (error) {
...
router.post("/vehicle/revoke", async function (req: any, res: any) {
  try {
    ...
    const revoked = await VehicleAuthService.revokeVehicle(
      parseInt(vehicleVin),
    );
    ...
    try {
      ...
      res.json({
        ...
        summary: {
          ...
          notProvisioned: vehicles.filter(
            (v) => v.authStatus === "not_provisioned",
          ).length,
        },
      });
    } finally {
...
router.post(
  "/vehicle/:vehicleVin/reset-rate-limit",
  async function (req: any, res: any) {
    try {
      const vehicleVin = parseInt(req.params.vehicleVin);
...
router.get(
  "/ratelimit/sliding-window/:vehicleVin",
  async function (req: any, res: any) {
    try {
      const vehicleVin = parseInt(req.params.vehicleVin);
      ...
      const stats = await RateLimitMonitor.getSlidingWindowStats(
        vehicleVin,
        endpoint,
      );
      res.json(stats);
      ...
      res
        .status(500)
        .json({ error: "Failed to get sliding window statistics" });
    }
  },
);
...
router.get("/ratelimit/sliding-window", async function (req: any, res: any) {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to get global sliding window statistics" });
  }
});

...
async function testSlidingWindow() {
  ...
  for (let i = 1; i <= 8; i++) {
    const result = await cache.checkRateLimitWithAnalytics(
      testKey,
      limit,
      windowSeconds,
    );
    ...
    console.log(
      `  🕐 Window Start: ${new Date(result.windowStart).toISOString()}`,
    );
    console.log(`  📊 Requests in Window: ${result.requestTimes.length}`);
    if (result.averageInterval) {
      console.log(
        `  ⏱️  Average Interval: ${Math.round(result.averageInterval)}ms`,
      );
    }

    if (!result.allowed) {
      console.log(
        `  🚫 Rate limited! Next allowed in ~${Math.ceil((result.resetTime - Date.now()) / 1000)}s`,
      );
      break;
    }
  ...
  result.requestTimes.forEach((time, index) => {
    ...
    console.log(
      `  ${index + 1}. ${new Date(time).toISOString()} (${secondsAgo}s ago)`,
    );
  });
